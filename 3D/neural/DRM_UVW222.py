...
def main():
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\u222.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\v222.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\w222.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    model_low.load_state_dict(torch.load('model_uvw22.mdl'))
    ...
    C01111 = torch.mean(C1 + C2 * u_x + C1 * v_y + C2 * w_z)
    print("C01111: ", C01111)
    ...
    for epoch in range(epochs + 1):
        ...
        loss_r2 = torch.squeeze((C1 - C01111) * v)
        loss_r3 = torch.squeeze((C3 * lu_y + C3 * lv_x) * u)
        loss_r4 = torch.squeeze((C2 * lu_x + C1 * lv_y + C2 * lw_z) * v)
        loss_r5 = torch.squeeze((C3 * lv_z + C3 * lw_y) * w)
        loss_r6 = torch.squeeze((C3 * u_y + C3 * v_x) * lu)
        loss_r7 = torch.squeeze((C2 * u_x + C1 * v_y + C2 * w_z) * lv)
        loss_r8 = torch.squeeze((C3 * v_z + C3 * w_y) * lw)
        ...
        if epoch > int(3 * epochs / 5):
            if torch.abs(loss) < best_loss:
                ...
                best_epoch = epoch
                torch.save(model.state_dict(), 'model_uvw222.mdl')
