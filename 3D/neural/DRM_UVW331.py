...
def main():
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\u331.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\v331.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\w331.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    C01111 = torch.mean(C3 + C3 * u_z + C3 * w_x)
    print("C01111: ", C01111)
    ...
    for epoch in range(epochs + 1):
        ...
        loss_r1 = 0.5 * (torch.squeeze(C1 * u_x * u_x) + torch.squeeze(C2 * v_y * u_x) + torch.squeeze(C2 * u_x * w_z)
                    ...
                    C1 * w_z * w_z))
        loss_r2 = torch.squeeze((C3 - C01111) * u)
        ...
        if epoch > int(3 * epochs / 5):
            if torch.abs(loss) < best_loss:
                ...
                best_epoch = epoch
                torch.save(model.state_dict(), 'model_uvw331.mdl')
