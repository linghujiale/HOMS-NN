...
def main():
    ...
    data = pd.read_csv(r"E:\pycharm\project6\3d\data\N22.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    # calculate a0
    xr_c = get_interior_points_c(80)
    xr_c = xr_c.to(device)
    xr_c.requires_grad_()
    thermal = func_thermal(xr_c, thermal1, thermal2, "conductivity").to(device)
    model_low = drrnn(in_N, m, out_N, depth).to(device)
    model_low.load_state_dict(torch.load('model_N2.mdl'))
    output_low = model_low(xr_c)
    grads = autograd.grad(outputs=output_low, inputs=xr_c,
    ...
    for epoch in range(epochs + 1):
        ...
        low_y = autograd.grad(outputs=output_low, inputs=xry,
                              grad_outputs=torch.ones_like(output_low),
                              create_graph=True, retain_graph=True, only_inputs=True)[0]
        ...
        loss_2 = torch.squeeze((thermal * low_y + thermal - a0) * output_r)
        loss_3 = torch.squeeze(thermal * output_low * u_y)

        loss_r = torch.mean(loss_1 - loss_2 + loss_3)
        loss = loss_r
