...
def main():
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\u311.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\v311.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\w311.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    C01111 = torch.mean(C2 + C2 * u_x + C2 * v_y + C1 * w_z)
    print("C01111: ", C01111)
    ...
    for epoch in range(epochs + 1):
        ...
        loss_r1 = 0.5 * (torch.squeeze(C1 * u_x * u_x) + torch.squeeze(C2 * v_y * u_x) + torch.squeeze(C2 * u_x * w_z)
                    ...
                    C1 * w_z * w_z))
        loss_r2 = torch.squeeze((C2 - C01111) * w)
        loss_r3 = torch.squeeze((C3 * lu_z + C3 * lw_x) * u)
        loss_r4 = torch.squeeze((C3 * lv_z + C3 * lw_y) * v)
        loss_r5 = torch.squeeze((C2 * lu_x + C2 * lv_y + C1 * lw_z) * w)
        loss_r6 = torch.squeeze((C3 * u_z + C3 * w_x) * lu)
        loss_r7 = torch.squeeze((C3 * v_z + C3 * w_y) * lv)
        loss_r8 = torch.squeeze((C2 * u_x + C2 * v_y + C1 * w_z) * lw)
        ...
        if epoch > int(3 * epochs / 5):
            if torch.abs(loss) < best_loss:
                ...
                best_epoch = epoch
                torch.save(model.state_dict(), 'model_uvw311.mdl')
