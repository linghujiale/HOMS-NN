...
def main():
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\u11.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\v11.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\w11.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    for epoch in range(epochs+1):
        ...
        loss_r2 = torch.squeeze(C1 * u_x) + torch.squeeze(C2 * v_y) + torch.squeeze(C2 * w_z)
        ...
        if epoch > int(3 * epochs / 5):
            if torch.abs(loss) < best_loss:
                ...
                best_epoch = epoch
                torch.save(model.state_dict(), 'model_uvw11.mdl')
