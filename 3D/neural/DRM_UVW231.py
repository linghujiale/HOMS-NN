...
def main():
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\u231.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\v231.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\w231.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    model_low.load_state_dict(torch.load('model_uvw31.mdl'))
    ...
    for epoch in range(epochs + 1):
        ...
        loss_r = torch.mean(loss_r1 - loss_r3 - loss_r4 - loss_r5 + loss_r6 + loss_r7 + loss_r8)
        loss = loss_r
        ...
        if epoch > int(3 * epochs / 5):
            if torch.abs(loss) < best_loss:
                ...
                best_epoch = epoch
                torch.save(model.state_dict(), 'model_uvw231.mdl')
