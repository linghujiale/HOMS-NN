...
def main():
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\u333.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\v333.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    data = pd.read_csv(r"E:\PyCharm\project6\3d\data\w333.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    C01111 = torch.mean(C1 + C2 * u_x + C2 * v_y + C1 * w_z)
    print("C01111: ", C01111)
    ...
    for epoch in range(epochs + 1):
        ...
        loss_r1 = 0.5 * (torch.squeeze(C1 * u_x * u_x) + torch.squeeze(C2 * v_y * u_x) + torch.squeeze(C2 * u_x * w_z)
                    ...
                    C1 * w_z * w_z))
        loss_r2 = torch.squeeze((C1 - C01111) * w)
        ...
        if epoch > int(3 * epochs / 5):
            if torch.abs(loss) < best_loss:
                ...
                best_epoch = epoch
                torch.save(model.state_dict(), 'model_uvw333.mdl')
