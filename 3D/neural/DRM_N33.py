...
def main():
    ...
    data = pd.read_csv(r"E:\pycharm\project6\3d\data\N33.csv")
    data = np.array(data)
    data = data[0, 0:27000]
    ...
    a0 = torch.mean(thermal + thermal * u_z)
    print("a0: ", a0)
    ...
    for epoch in range(epochs + 1):
        ...
        low_z = autograd.grad(outputs=output_low, inputs=xrz,
                              grad_outputs=torch.ones_like(output_low),
                              create_graph=True, retain_graph=True, only_inputs=True)[0]
        ...
        loss_2 = torch.squeeze((thermal * low_z + thermal - a0) * output_r)
        loss_3 = torch.squeeze(thermal * output_low * u_z)
        ...
        if epoch > int(3 * epochs / 5):
            if torch.abs(loss) < best_loss:
                ...
                best_epoch = epoch
                torch.save(model.state_dict(), 'model_N33.mdl')
