...
def main():
    ...
    data = pd.read_csv(r"E:\pycharm\project6\2d\data\N22.csv")
    data = np.array(data)
    data = data[0, 0:10000]
    data = data.reshape(100, 100)
    datau = torch.tensor(data).to(device)
    datauN11 = torch.norm(datau, p=2)

    model_N1 = drrnn(in_N, m, depth, out_N).to(device)
    model_N1.load_state_dict(torch.load('model_N2.mdl'))
    ...
    xr.requires_grad_()
    output_r_a0 = model_N1(xr)
    grads = autograd.grad(outputs=output_r_a0, inputs=xr,
                          grad_outputs=torch.ones_like(output_r_a0),
                          create_graph=True, retain_graph=True, only_inputs=True)[0]
    u_x, u_y = torch.chunk(grads, 2, 1)
    a0 = torch.mean(thermal + thermal * u_y)
    print("a0: ", a0)
    ...
    for epoch in range(epochs+1):
        ...
        grads = autograd.grad(outputs=output_r, inputs=xr,
                              ...
                              create_graph=True, retain_graph=True, only_inputs=True)[0]
        N11_x, N11_y = torch.chunk(grads, 2, 1)

        output_N1_r = model_N1(xr)
        grads_N1 = autograd.grad(outputs=output_N1_r, inputs=xr,
                                 grad_outputs=torch.ones_like(output_N1_r),
                                 create_graph=True, retain_graph=True, only_inputs=True)[0]
        N1_x, N1_y = torch.chunk(grads_N1, 2, 1)

        loss_r1 = torch.mean(0.5 * torch.sum(thermalxr * torch.pow(grads, 2), dim=1))
        loss_r2 = torch.mean(torch.squeeze((thermalxr * N1_y + thermalxr - a0) * output_r))
        loss_r3 = torch.mean(torch.squeeze(thermalxr * output_N1_r * N11_y))
        ...
        if epoch % 500 == 0:
            ...
            error_u = datau - pred
            UL2 = torch.norm(error_u, p=2) / datauN11

            print('epoch:', epoch, 'loss:', loss.item(), 'loss_r:', loss_r.item(),
                  'lr', optimizer.state_dict()['param_groups'][0]['lr'],
                  "NL2Rerror: ", UL2.item())

    torch.save(model.state_dict(), 'model_N22.mdl')
