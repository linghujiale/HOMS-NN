...
def func_thermal(X, conductivity1, conductivity2, func):
    ...
    if func == "conductivity":
        output = (1 / (1 + torch.exp(- k * ((x - 0.5) ** 2 + (y - 0.5) ** 2 - r ** 2)))) * (
...
def main():
    ...
    data = pd.read_csv(r"E:\pycharm\project6\2d\data\N2.csv")
    data = np.array(data)
    data = data[0, 0:10000]
    ...
    for epoch in range(epochs + 1):
        ...
        loss_r2 = torch.squeeze(thermalxr * u_y)
    ...
    torch.save(model.state_dict(), 'model_N2.mdl')
