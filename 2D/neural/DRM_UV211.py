...
def main():
    ...
    data = pd.read_csv(r"E:\pycharm\project6\2d\data\u211.csv")
    data = np.array(data)
    data = data[0,0:10000]
    ...
    data = pd.read_csv(r"E:\pycharm\project6\2d\data\v211.csv")
    data = np.array(data)
    data = data[0,0:10000]
    data = data.reshape(100, 100)
    ...
    model_uv.load_state_dict(torch.load('model_uv11.mdl'))
    ...
    xr_c = xr_c.to(device)
    C1111 = C1111.to(device)
    C1122 = C1122.to(device)
    xr_c.requires_grad_()
    ...
    C01111 = torch.mean(C1122 + C1122 * u_x + C1111 * v_y)
    print("C01111: ", C01111)
    ...
    for epoch in range(epochs+1):
        ...
        loss_r2 = torch.squeeze((C1122 - C01111) * v)
        loss_r3 = torch.squeeze((C1122 * lu_x + C1111 * lv_y) * v)
        loss_r4 = torch.squeeze((C1212 * (lu_y + lv_x)) * u)
        loss_r5 = torch.squeeze((C1122 * u_x + C1111 * v_y) * lv)
        loss_r6 = torch.squeeze((C1212 * (u_y + v_x)) * lu)
    ...
    torch.save(model.state_dict(), 'model_uv211.mdl')
